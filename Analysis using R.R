###############################################################################
#
# Turtle Games - Exploratory Data Analysis in R
#
###############################################################################
################################## THE SCENARIO ###############################
###############################################################################

# Turtle Games’ sales department has historically preferred to use R for sales
# analyses. R will be used to perform exploratory data analysis (EDA) to
...
# Updated path reflecting the OneDrive location
file_path <- "C:/Users/tamar/OneDrive/Desktop/Turtle Games/turtle_reviews_clean.csv"

# Load the file
reviews_r <- read.csv(file_path, header = TRUE)
...
# 1. Keep only the numeric data
numeric_data <- reviews_r %>% select(where(is.numeric))

# Compute the actual correlation values, ignoring any missing data
correlation_matrix <- cor(numeric_data, use = "complete.obs")

# Ensure the plotting library is loaded ---
# install.packages("corrplot") # Uncomment this line if you get a "could not find function corrplot" error
# library(corrplot)
...
###############################################################################
# 
#  Multiple Linear Regression in R – Turtle Games
#
###############################################################################
